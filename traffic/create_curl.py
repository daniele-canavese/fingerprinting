...
parser = ArgumentParser(description="Generates the cURL pcap file")
parser.add_argument("--app", default="curl-7.61.0", help="the name of the application")
...
for i in urls:
    print("%d) %s" % (count, i))
    system("curl --silent --output /dev/null \"%s\"" % i)
    count += 1
sleep(delay)
