...
parser = ArgumentParser(description="Generates the HULK pcap file")
parser.add_argument("--app", default="hulk-1.0", help="the name of the application")
...
for i in urls:
    print("%d) %s" % (count, i))
    system("python hulk/hulk.py %s safe > /dev/null" % i)
    count += 1
sleep(delay)
